...
class Strings {
  PVector pos;
  Strings(int i){
       pos = new PVector(width/3+i*3-floor(i/nop)*(nop*3), height/2, -(floor(i/nop)*20));
  }
  public void update(int i){
    pos.y = (pow(((floor(i/nop)-10)/1.5),2)+5)*cos((width/2-pos.x)/(95.5/(2*floor(i/nop)+1)))*cos(tiempo*(floor(i/nop)+1)*0.25+HALF_PI)+height/2;
